...
import com.winsun.item.modular.system.dao.MenuMapper;
...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
    ...
    @Override
    public List<MenuNode> getMenusByRoleIds(List<Integer> roleIds, String pcode) {
        return this.baseMapper.getMenusByRoleIds(roleIds, pcode);
    }

	@Override
	public void setStatus(Long menuId, int status) {
		
		Menu menu = menuMapper.selectById(menuId);
        //当前菜单
        this.baseMapper.setStatus(menuId, status);
        
        if (status == ManagerStatus.CLOSE.getCode()) {
        	//关闭所有子菜单
            Wrapper<Menu> wrapper = new EntityWrapper<>();
            wrapper = wrapper.like("pcodes", "%[" + menu.getCode() + "]%");
            List<Menu> menus = menuMapper.selectList(wrapper);
            for (Menu temp : menus) {
                this.baseMapper.setStatus(temp.getId(), status);
            }
        }
	}
}
